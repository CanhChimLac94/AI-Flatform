...
export function PageHeader({
  ...
  const hasTrailing = Boolean(actions || badge);

  return (
    <header className="shrink-0 border-b border-gray-700 px-4 sm:px-6 py-3">
      <div
        className={`flex min-w-0 gap-2.5 md:items-center md:gap-3 ${
          center ? "flex-col md:flex-row" : "flex-row items-center"
        }`}
      >
        <div
          className={`flex items-center gap-2 min-w-0 ${
            center ? "justify-between md:contents" : "flex-1"
          }`}
        >
          <div
            className={`flex items-center gap-2 sm:gap-3 min-w-0 ${
              center ? "md:order-1 md:shrink-0" : "flex-1"
            }`}
          >
            <MobileMenuButton />
